...
function itemplacer:GetItem(item)
    ...
    if placerItem ~= nil and placerItem.replica.inventoryitem ~= nil and
        placerItem.components then
    ...
    if  type(item) == "string" then
        if  PrefabExists(item .. "_placer") then
            return item .. "_placer"
        end
    end
end
function itemplacer:ShowPlacer(pos)
    ...
    for i, v in ipairs(pos) do
        local placerName = self:GetItem(v.prefab)
        if placerName then
            local deployPlacer = SpawnPrefab(placerName, v.skin,nil,v.skin and TheNet:GetUserID() or nil)
            if deployPlacer then
                local rot = (i - 1) * ThePlayer.components.deploydata.step
