...
function RCSPBWidget:DestDeleteListItem()
    ...
    dest.parmname:SetPosition(-18, 0, 0)
...
function RCSPBWidget:DestNormalListItem()
    ...
    dest.parmname = dest:AddChild(Text(BODYTEXTFONT, 24))
    dest.parmname:SetPosition(5, 0, 0)
    dest.parmname:SetHAlign(ANCHOR_LEFT)
...
function RCSPBWidget:MakeDelScreen()
...
function RCSPBWidget:MakeNormalScreen()
    local function ScrollWidgetsCtor(context, index)
        local widget = Widget("widget-" .. index)
        widget:SetOnGainFocus(function()
...
function RCSPBWidget:Close() self:Kill() end

function RCSPBWidget:OnUpdate() end

function RCSPBWidget:OnRawKey(key, down, ...) end

function RCSPBWidget:OnControl(control, down)
    if RCSPBWidget._base.OnControl(self, control, down) then return true end
    if not down and control == CONTROL_CANCEL then self:Close()return true end
end

function RCSPBWidget:GetHelpText() end
