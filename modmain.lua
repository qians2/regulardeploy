...
function RCMakeWidgetMovable(s, name, pos, data) -- 使UI可移动 
...
GLOBAL.RCMakeWidgetMovable = RCMakeWidgetMovable
AddComponentPostInit("playercontroller", function(self, inst)
    if inst ~= _G.ThePlayer then return end
    ThePlayer:AddComponent("deploydata")
end)

local function HighlightCenterDeploy()
    local center_x, center_z
    local rc = ThePlayer.components.deploydata
    ...
    rc.center_x = center_x
    rc.center_z = center_z
    if not rc.guideline then rc.guideline = SpawnPrefab("deployrange") end
    if not rc.guideline:IsValid() then rc.guideline = SpawnPrefab("deployrange") end
    local scale = math.sqrt(rc.radius * 0.16)
    rc.guideline.Transform:SetPosition(center_x, 0, center_z)
    rc.guideline.Transform:SetScale(scale, scale, scale)
    rc.guideline:Show()
    if  not rc.gridplacer then
        if PrefabExists("buildgridplacer") then
            rc.gridplacer = SpawnPrefab("buildgridplacer")
            rc.gridplacer.AnimState:PlayAnimation("on", true)
            rc.gridplacer.Transform:SetScale(1.7, 1.7, 1.7)
        else
            rc.gridplacer = SpawnPrefab("gridplacer")
        end
    end
    if  not rc.gridplacer:IsValid() then         
        if PrefabExists("buildgridplacer") then
        rc.gridplacer = SpawnPrefab("buildgridplacer")
        rc.gridplacer.AnimState:PlayAnimation("on", true)
        rc.gridplacer.Transform:SetScale(1.7, 1.7, 1.7)
    ...
    rc.gridplacer:Show()
    rc.gridplacer.Transform:SetPosition(center_x, 0, center_z)
    rc.candeploy = true
end
...
local function show_position(self)
        ...
        self.dp = self.top_root:AddChild(DPWidget(ThePlayer))
        local screenw_full,_ = TheSim:GetScreenSize()
        self.dp.num:SetPosition( - screenw_full / 2 + 200, -20)
end
...
AddClassPostConstruct("screens/playerhud", function(self)
    ...
    local rc = ThePlayer.components.deploydata
    local ut = require("deployutil")(ThePlayer)
	self.inst:DoTaskInTime(0.1, function()
		...
		function self:OnRawKey(key, down, ...)  
            ...
            local key1,key2,key3 = rc:GetRwKey()--界面,摆放,预 览
            ...
            elseif key == key2 and not down then
                ...
                if  TheInput:IsControlPressed(CONTROL_FORCE_TRADE) then
                    if rc.gridplacer and rc.gridplacer:IsValid() then rc.gridplacer:Hide() end
                    if rc.guideline and rc.guideline:IsValid() then rc.guideline:Hide() end
                    rc.candeploy = false
                elseif TheInput:IsControlPressed(CONTROL_FORCE_STACK) then
                    HighlightCenterDeploy()
                elseif rc.center_x and rc.center_z and rc.candeploy then
                    ut:StartAutoDeploy()
                end
        ...
        function self:OnControl(control, down, ...)
            ...
            if rc.recordmode and TheInput:IsControlPressed(CONTROL_FORCE_INSPECT) and down then
                local entity = ConsoleWorldEntityUnderMouse()
                if  control == CONTROL_ACCEPT then
                    if  entity then
                        rc:SelectEntity(entity)
                        return false
                    end
                end
            end
